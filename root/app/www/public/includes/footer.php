    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content bg-dark" style="border: grey solid 1px;">
            ...
            <div class="modal-body" style="border: grey solid 1px;">
                ...
                <span class="text-white">I'm gathering everything needed to complete the request, give me just a
                    moment...</span>
                </p>
            </div>
        ...
        <div class="modal-content bg-dark" style="border: grey solid 1px;">
            <div class="modal-header" style="border: grey solid 1px;">
                <h5 class="modal-title text-info">
                    <div id="massTrigger-spinner" class="spinner-border text-info" style="margin-right: 1em;"></div>
                    Mass Trigger: <span id="triggerAction"></span>
                </h5>
            </div>
            <div class="modal-body" style="border: grey solid 1px;">
...
<!-- Trivy scan modal -->
<div class="modal fade" id="trivyModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content bg-secondary">
            <div class="modal-header">
                <h5 class="modal-title" id="trivyModalTitle">Trivy Scan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body" id="trivyModalBody">
            </div>
...
<!-- Trivy mass scan modal -->
<div class="modal fade" id="trivyScanModal" style="z-index: 9999 !important; overflow: auto;" data-bs-backdrop="static">
    <div class="modal-dialog" style="max-width: 1000px">
        <div class="modal-content bg-dark" style="border: grey solid 1px;">
            <div class="modal-header" style="border: grey solid 1px;">
                <h5 class="modal-title text-info">
                    <div id="trivyScan-spinner" class="spinner-border text-info" style="margin-right: 1em;"></div>
                    Trivy Scan
                </h5>
            </div>
            <div class="modal-body" style="border: grey solid 1px;">
                <div id="trivyScan-header"></div>
                <div id="trivyScan-results" style="max-height: 600px; overflow: auto;"></div>
            </div>
            <div class="modal-footer" align="center">
                <button id="trivyScan-close-btn" style="display: none;" type="button" class="btn btn-outline-danger" data-bs-dismiss="modal">Close</button>
            </div>
        </div>
    </div>
</div>
...
<div id="registryLoginDiv" style="display: none;">
    <div class="col-lg-12">
        ...
        <div class="row mt-2">
            <div class="col-sm-12"><button class="btn btn-outline-success" onclick="registryLogin()">Login</button>
            </div>
        </div>
    </div>
</div>
...
<div id="containerFrequencyHelpDiv" style="display: none;">
    ...
    An example of a cron for every hour would be: <code>0 * * * *</code> and you can generate a cron for just about any
    variation using <a href="https://crontab.guru/" target="_blank" class="text-info">Cronitor</a><br><br>
    <span class="text-danger">NOTES:</span>
    <ul>
        <li>The minute section will always be set to 0 (zero) for a minimum check time of 1 hour, be respectful of
            external API usage.</li>
        <li>Using an invalid cron syntax will default to daily at midnight <code<?= DEFAULT_CRON ?></code>
        </li>
    </ul>
</div>
...
<div id="frequencyCronEditorDiv" style="display: none;">
    ...
    <span class="text-danger">NOTES:</span>
    <ul>
        <li>If the cron output does not meet your requirements, you can enter your own by overwriting the output box.
        </li>
    </ul>
</div>
...
<div id="dockwatchWarningText" style="display: none;">
    Dockwatch has a different method for updating and restarting its self, for this reason it is ignored in the check
    all action. It will clone its self and create a container named <code>dockwatch-maintenance</code> since it can not
    do things to its self.<br><br>
    The <code>dockwatch-maintenance</code> container needs a port, by default it is <code>9998</code> but you can change
    it in the settings. If you use a static ip for your containers, make sure and set an ip for it as well.<br><br>
    ...
    <br>
    Consider not running the dockwatch update time at the same time as other containers. When dockwatch starts its
    update process, everything after it will be ignored since it is stopping its self!
    <hr>
    <?php if ($_SESSION['activeServerId'] != APP_SERVER_ID) { ?>
        Remote control of self restarts and updates is not supported.
    <?php } else { ?>
        Wait 30-45 seconds after using these buttons and refresh the page so the process outlined above can complete. If you
        have notifications enabled you will see the maintenance container start and shortly after the dockwatch container
        start.<br><br>
        <center>
